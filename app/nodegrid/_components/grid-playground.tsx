...
function FloatingPanel({
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      if (hasMoved) {
        ...
        const cellX = Math.floor(centerX / gridCellSize);
        const cellY = Math.floor(centerY / gridCellSize);
  ...
  const handleTouchResizeStart = (touch: Touch | React.Touch, edge: ResizeEdge) => {
    ...
    const handleTouchResizeMove = (moveEvent: TouchEvent) => {
      ...
      const widthCells = Math.floor(newWidth / gridCellSize);
      const heightCells = Math.floor(newHeight / gridCellSize);
  ...
  const handleResizeStart = (e: React.MouseEvent, edge: ResizeEdge) => {
    ...
    const handleResizeMove = (moveEvent: MouseEvent) => {
      ...
      if (shiftHeld) {
        newWidth = Math.max(gridCellSize, snapToGrid(newWidth));
        newHeight = Math.max(gridCellSize, snapToGrid(newHeight));
      }
...
export type GridPlaygroundProps = {
  initialGridType?: GridType;
  initialTheme?: 'dark' | 'light';
  initialAccentHex?: string;
  initialGridCellSize?: number;
  initialStrokeScale?: number;
};

export function GridPlayground({
  initialGridType = 'rectangular',
  initialTheme = 'dark',
  initialAccentHex = '#2563eb',
  initialGridCellSize = DEFAULT_GRID_CELL_SIZE,
  initialStrokeScale = DEFAULT_STROKE_SCALE,
}: GridPlaygroundProps = {}) {
  ...
  const [accentHex, setAccentHex] = useState<string>(initialAccentHex);
  const [gridCellSize, setGridCellSize] = useState(() =>
    clampNumber(initialGridCellSize, GRID_CELL_SIZE_MIN, GRID_CELL_SIZE_MAX),
  );
  const [strokeScale, setStrokeScale] = useState(() =>
    clampNumber(initialStrokeScale, STROKE_SCALE_MIN, STROKE_SCALE_MAX),
  );

  const defaultPanelSize = useMemo(
    () => ({ width: gridCellSize * 4, height: gridCellSize * 4 }),
    [gridCellSize],
  );
  ...
  useEffect(() => {
    setGridCellSize(clampNumber(initialGridCellSize, GRID_CELL_SIZE_MIN, GRID_CELL_SIZE_MAX));
  }, [initialGridCellSize]);

  useEffect(() => {
    setStrokeScale(clampNumber(initialStrokeScale, STROKE_SCALE_MIN, STROKE_SCALE_MAX));
  }, [initialStrokeScale]);
  ...
  const copyReactTailwindCode = useCallback(async () => {
    ...
    const query = new URLSearchParams({
      theme,
      accent: accentHex,
      cell: String(gridCellSize),
      stroke: String(strokeScale),
    }).toString();
    const isolatedUrl = origin ? `${origin}${isolatedPath}?${query}` : `${isolatedPath}?${query}`;
    const label = gridTypeLabel(gridType);
    const fence = '```';
    const routeLines = NODE_GRID_ORDER.map((t) => {
      const base = origin ? `${origin}/nodegrid/` : '/nodegrid/';
      return ` * ${base}${gridTypeToSlug(t)}?theme=dark&accent=%232563eb&cell=40&stroke=1`;
    }).join('\n');

    const code = `/**
 * Robot Components — Node Grid / "${label}"
 * ---------------------------------------------------------------------------
 * gridType (internal):  ${gridType}
 * Live preset URL (full playground — panels, connections, hover sounds):
 *   ${isolatedUrl}
 * Upstream source tree: ${NODEGRID_GITHUB_BASE}
 *
 * WHAT THIS IS
 * Canvas 2D node grid + optional WebGL film grain (NoiseOverlay). Floating
 * panels, connections, slice-to-cut, and sound effects live in
 * grid-playground.tsx. Not pure Tailwind — copy the TS modules below.
 *
 ...
 * - app/nodegrid/_components/grid-playground.tsx   (full interactive demo)
 *
 * URL QUERY (optional on /nodegrid/[slug]) — preserves theme & layout:
 *   theme=dark|light
 *   accent=#rrggbb (URL-encoded, e.g. %232563eb)
 *   cell=<number>   grid spacing px (${GRID_CELL_SIZE_MIN}–${GRID_CELL_SIZE_MAX})
 *   stroke=<number> line-width multiplier (${STROKE_SCALE_MIN}–${STROKE_SCALE_MAX})
 *
 * TUNABLE PROPS — GridPlayground (see grid-playground.tsx)
 * - initialGridType: GridType
 * - initialTheme:    'dark' | 'light'
 * - initialAccentHex: '#rrggbb'
 * - initialGridCellSize: number   // default ${DEFAULT_GRID_CELL_SIZE}
 * - initialStrokeScale: number     // default ${DEFAULT_STROKE_SCALE}
 *
 * TUNABLE PROPS — DotGridCanvas (lower-level)
 * - gridCellSize, strokeScale (plus panel/pulse/connection props like the playground)
 *
 * FULL PLAYGROUND (recommended — matches demo site)
 */
import { GridPlayground } from './_components/grid-playground'; // adjust path after copy

export function MyNodeEditorPage() {
  return (
    <GridPlayground
      initialGridType="${gridType}"
      initialTheme="${theme}"
      initialAccentHex="${accentHex}"
      initialGridCellSize={${gridCellSize}}
      initialStrokeScale={${strokeScale}}
    />
  );
}

/**
 * BACKGROUND-ONLY (no panels): import NodeGridIsolated from
 * ./_components/node-grid-isolated.tsx and pass gridCellSize / strokeScale
 * there as well (it forwards to DotGridCanvas).
 *
 * Indexed preset routes (swap slug): same query pattern as above.
${routeLines}
 */
${fence}tsx
// Optional Tailwind chrome (not the grid):
...
`;
  ...
  }, [accentHex, gridCellSize, gridType, strokeScale, theme]);
  ...
  useEffect(() => {
    ...
    const x = (window.innerWidth - defaultPanelSize.width) / 2;
    const y = (window.innerHeight - defaultPanelSize.height) / 2;
  ...
  }, [connections, floatingPanels, lineSegmentsIntersect, gridCellSize]);
  ...
  const handleGridClick = useCallback((e: React.MouseEvent) => {
    ...
    const x = e.clientX - defaultPanelSize.width / 2;
    const y = e.clientY - defaultPanelSize.height / 2;
  ...
  const handleGridTouch = useCallback((e: React.TouchEvent) => {
    ...
    const x = touch.clientX - defaultPanelSize.width / 2;
    const y = touch.clientY - defaultPanelSize.height / 2;
    const id = `floating-panel-${panelIdCounter.current++}`;
  ...
  const handleConnectionDragEnd = useCallback((fromPanelId: string, toPanelId: string | null, dropX: number, dropY: number) => {
    ...
    if (toPanelId) {
      ...
      const x = dropX - defaultPanelSize.width / 2;
      const y = dropY - defaultPanelSize.height / 2;
      ...
      >
        ...
        >
          <label
            ...
            htmlFor="nodegrid-cell-size"
          >
            <span style={{ width: 52, flexShrink: 0 }}>Cell</span>
            <input
              id="nodegrid-cell-size"
              type="range"
              ...
              style={{ width: 120 }}
            />
            <span style={{ width: 36, textAlign: 'right', fontFamily: 'ui-monospace, monospace', color: 'var(--app-fg)' }}>{gridCellSize}px</span>
          </label>
          <label
            style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 11, color: 'var(--muted-fg)', cursor: 'pointer' }}
            htmlFor="nodegrid-stroke-scale"
          >
            <span style={{ width: 52, flexShrink: 0 }}>Stroke</span>
            <input
              id="nodegrid-stroke-scale"
              type="range"
              min={25}
              max={400}
              step={5}
              ...
              aria-label="Line stroke scale"
              style={{ width: 120 }}
            />
            <span style={{ width: 36, textAlign: 'right', fontFamily: 'ui-monospace, monospace', color: 'var(--app-fg)' }}>{strokeScale.toFixed(2)}×</span>
          </label>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <Link
            href={`/nodegrid/${gridTypeToSlug(gridType)}?${new URLSearchParams({
              theme,
              accent: accentHex,
              cell: String(gridCellSize),
              stroke: String(strokeScale),
            ...
            title="Open this preset in a new tab (full playground: panels, connections, sounds). Theme and sizing are carried in the URL."
          >
            Open preset
          </Link>
        </div>
